...
static inline void sendBit(uint8_t value) {
    ...
    HAL_GPIO_WritePin(SPI_DATA_GPIO_Port,SPI_DATA_Pin, value ? GPIO_PIN_SET : GPIO_PIN_RESET);//digitalWrite(PIN_SPI_DATA, value);
    DWT_Delay_us(1);//delayMicroseconds(1);
...
static void sendSlaveSelect(uint8_t value) {
	HAL_GPIO_WritePin(SPI_SLAVE_SELECT_A_GPIO_Port,SPI_SLAVE_SELECT_A_Pin,value ? GPIO_PIN_SET : GPIO_PIN_RESET);
    HAL_GPIO_WritePin(SPI_SLAVE_SELECT_B_GPIO_Port,SPI_SLAVE_SELECT_B_Pin,value ? GPIO_PIN_SET : GPIO_PIN_RESET);
    DWT_Delay_us(1);
